    ...
    public partial class ThiTracNghiem_VIEW : Form
    {
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            try
            {
                if (cls.checkBD(int.Parse(cbChuDe.EditValue.ToString()), userName))
                {
                    ...
                    bd.Nop_Bai = false;
                    if (cbChuDe.Text.ToString().Contains("thi thử") == false && cbChuDe.Text.ToString().Contains("Thi thử") == false)
                    {
                        cls.insertBD(bd);
                    }
        ...
        private void simpleButton2_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception)
            {
                ...
                if (MessageBox.Show("Nộp bài không thành công, vui lòng kiểm tra lại kết nối","Cảnh báo",MessageBoxButtons.RetryCancel)==DialogResult.Retry)
                {
                    MessageBox.Show("BEN Ngoai 2");
                }
                MessageBox.Show("BEN Ngoai"); 
            }
        }
        ...
        public void exportExcel()
        {
            ...
            foreach (var item in lst)
            {
                ...
                ws.Cell("B" + row.ToString()).Value = item.Ma_Chu_De;
                ws.Cell("C" + row.ToString()).Value = item.Noi_Dung;
                ws.Cell("D" + row.ToString()).Value = item.Dap_An_A;
                ws.Cell("E" + row.ToString()).Value = item.Dap_An_B;
                ws.Cell("F" + row.ToString()).Value = item.Dap_An_C;
                ws.Cell("G" + row.ToString()).Value = item.Dap_An_D;
                ws.Cell("H" + row.ToString()).Value = item.Dap_An_Dung;
  ws.Cell("I" + row.ToString()).Value = item.Dap_An_HS;
  ws.Cell("J" + row.ToString()).Value = item.Nguoi_Lam;
  ws.Cell("K" + row.ToString()).Value = item.Thoi_Gian;
  ws.Cell("L" + row.ToString()).Value = item.id_Tai_Khoan_HS;
                row++;
            }
        ...
        public void nopbai() {
            try
            {
                if (cbChuDe.Text.ToString().Contains("thi thu") == false)
                {

                    cls.insertDe_Hs(lst);
                    cls.updateBatDau(userName, int.Parse(cbChuDe.EditValue.ToString())); disable();
                }
                else {  }
                timer1.Enabled = false;
